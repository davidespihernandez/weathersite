...
We use Django cache module to save the list (actually a queue) of request times done by each requesting IP and calculate whether the limit is exceeded or not. 
By default, Django uses a local-memory cache (per process, thread safe) which is convenient for a test environment. 
The cache backend can be changed easily to more performant and scalable cache backends like Memcached. 
...
The cache contents for each cache key is a queue that contains the request times for that cache key (IP address), ordered from newer (first item) to older (last item). 
New request times are inserted at the beginning of the queue.
...
3. All items in the queue older than 60 seconds (the default duration we use) are deleted. This is done looping from the end of the queue (where the older request times are).
4. The new request time (current time) is inserted at the beginning of the queue.
5. If after the previous step we have more elements in the queue than the maximum number of requests per minute allowed, then a `RateExceeded` exception is raised. This is a Django REST framework API exception that will be translated into an HTTP 429 in the final response.
6. The cache value is updated with the queue, setting an expiration time of 60 seconds fot that cache entry. The cache will be cleaned automatically after that time, but everytime a new request is added, the expiration time will be extended 60 seconds, and older request will be deleted from the queue.
...
* Apply rate limit to authenticated users. 
  In the case of authenticated users we can simply use the user id as the cache key, and also establish different rates for authenticated and anonymous requests. 
  Limitations could also be applied to roles, and in this case we should check several cache entries (one for the user and another for the role).  
...
If you select the anonymous limited request and click on "Send", you'll' see the new headers in the response headers.

![Test result](Postman_test.png "Test result")

#### 4.2.1 Authenticated requests
The authentication system we use is JWT tokens, so in order to make an authenticated request you'll have first to retrieve a valid JWT token using the authentication endpoint. 
...
Postman allows you to run a request several times, so you can check the rate limitation actually works. 
In order to do that:

- Click on the ellipsis button on the collection row, and select "Run collection".
- Select only the anonymous limited request.
- Set 61 iterations.
- Click on run.
- You'll see the last request fails with a 429.

Of course, you can send the request manually 61 times and check how the headers change on each request, but it will take more time... 
